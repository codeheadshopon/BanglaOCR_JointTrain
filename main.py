...
def paint_text(text, w, h,  rotate=False, ud=True, multi_fonts=False):
    ...
    a = cv2.adaptiveThreshold(a, 1, cv2.ADAPTIVE_THRESH_MEAN_C, \
        ...
        for i in range(size):
            ...
            if train and i > size - 4:
                ...
                if K.image_data_format() == 'channels_first':
                    ...
                    a= self.paint_func('',)[0, :, :].T
                    b= self.paint_func('',)[0, :, :]

                    X_data[i, 0:self.img_w, :, 0] = self.paint_func(self.X_text[index + i])[0, :, :].T
                    # print(X_data[i,0:564,:,0])
...
def train(run_name, start_epoch, stop_epoch, img_w,type_t):
    ...
    print(x_data.shape)
    outputs = {'ctc': np.zeros([len(X_data)])}

    model.fit([np.array(x_data), np.array(labels), np.array(input_len), np.array(LabelLengh)],
              outputs, batch_size=12, epochs=100, verbose=1,
              validation_data=(
                  [np.array(x_data), np.array(labels), np.array(input_len), np.array(LabelLengh)]
                  , outputs))
